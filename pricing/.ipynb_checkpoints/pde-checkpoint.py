...
def crank_nicolson(
    ...
    for n in range(N_t):
        ...
        bc = (Smax - K * np.exp(-r*(t-dt))) if is_call else (K * np.exp(-r*(t-dt)))
        rhs[-1] -= gamma[-1] * bc
...
def crank_nicolson_barrier(
    ...
    A  = np.zeros((3, N_S-1))
    Bm = np.zeros((3, N_S-1))
    A[0,1:]   = -gamma[:-1];   A[1] = 1 - beta;    A[2,:-1] = -alpha[1:]
    Bm[0,1:]  =  gamma[:-1];   Bm[1] = 1 + beta;   Bm[2,:-1] = alpha[1:]
...
def rr_down_out_call(
    ...
    Bterm = (
        S0*(B/S0)**(2*mu) * norm.cdf(y1)
        - K*np.exp(-r*T)*(B/S0)**(2*mu - 2) * norm.cdf(y2)
    )
